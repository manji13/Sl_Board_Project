        ...
        <div className="flex-1 flex flex-col items-center justify-center min-h-[400px] text-center bg-white p-10">
          ...
          {searchTerm ? (
            ...
            <>
              ...
              <div className="text-[15px] text-[#94A3B8]">
                Click "Add New Coach" to get started
              </div>
            </>
          )}
        </div>
        ...
        {filteredCoaches.map((coach, idx) => (
          ...
          >
            ...
            <div className="flex flex-col gap-1">
              <div className="font-bold text-[16px] text-[#1A1A2E]">
                {coach.name || "—"}
              </div>
            ...
            <div className="flex flex-wrap gap-[6px]">
              {getDesig(coach).map((d, i) => (
                <span
                  key={i}
                  className="px-3 py-1 rounded-full text-[12px] font-semibold tracking-[0.3px] bg-[#EFF6FF] text-[#1D4ED8] border border-[#BFDBFE]"
                >
                  {d}
                </span>
              ))}
              {getDesig(coach).length === 0 && (
                <span className="px-3 py-1 rounded-full text-[12px] font-semibold tracking-[0.3px] bg-gray-100 text-gray-500">
                  —
                </span>
              )}
            </div>
            ...
            <div className="flex gap-2 justify-center">
              <button
                className="w-[38px] h-[38px] rounded-[10px] flex items-center justify-center text-[16px] transition shrink-0 bg-[#F1F5F9] text-[#4A5568] hover:bg-[#E2E8F0] hover:scale-105"
                title="View Details"
              ...
              <button
                className="w-[38px] h-[38px] rounded-[10px] flex items-center justify-center text-[16px] transition shrink-0 bg-[#F0E4C2] text-[#92640A] hover:bg-[#E8D49A] hover:scale-105"
                title="Edit Coach"
                onClick={() => navigate(`/edit/${coach._id}`)}
              >
                ✏️
              </button>
              <button
                className="w-[38px] h-[38px] rounded-[10px] flex items-center justify-center text-[16px] transition shrink-0 bg-[#FFE5E7] text-[#C1121F] hover:bg-[#FFC9CC] hover:scale-105"
                title="Delete Coach"
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          backgroundImage:
            "radial-gradient(circle at 10% 20%, rgba(201,168,76,0.06) 0%, transparent 50%), radial-gradient(circle at 90% 80%, rgba(45,106,79,0.06) 0%, transparent 50%)",
        }}
      >
        {/* Topbar */}
        ...
        <div className="max-w-[1400px] w-full mx-auto px-4 lg:px-6 py-[30px] flex-1">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-[30px] flex-wrap gap-5">
            ...
            <div className="flex flex-col md:flex-row gap-[15px] items-center w-full md:w-auto">
              <div className="relative w-full md:min-w-[320px]">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-[#4A5568] text-[16px] pointer-events-none">
                  🔍
                </span>
                <input
                ...
                {searchTerm && (
                  <button
                    className="absolute right-3 top-1/2 -translate-y-1/2 bg-transparent text-[#94A3B8] text-[18px] w-6 h-6 flex items-center justify-center rounded-full transition hover:bg-[#F1F5F9] hover:text-[#1A1A2E]"
              ...
              >
                <div className="w-[22px] h-[22px] bg-white/15 rounded-md flex items-center justify-center text-[16px] font-semibold transition group-hover:bg-[#1A1A2E]/15">
                  ＋
                </div>
                Add New Coach
              </button>
          ...
          <div className="bg-white border-2 border-[#E8E0D0] rounded-[20px] overflow-hidden shadow-[0_4px_24px_rgba(26,26,46,0.08)] flex flex-col min-h-[500px] w-full">
            ...
            <div className="grid grid-cols-[50px_1fr_1fr_100px] md:grid-cols-[60px_1.2fr_1.5fr_1fr_0.8fr_140px] lg:grid-cols-[70px_1.2fr_1.8fr_1fr_0.8fr_160px] px-4 md:px-5 lg:px-6 py-4 lg:py-[18px] bg-[#1A1A2E] gap-3 lg:gap-[15px] items-center shrink-0">
              ...
              <div className="hidden md:block text-[12px] font-bold tracking-[1.5px] uppercase text-white/60">
                District
              </div>
              <div className="hidden md:block text-[12px] font-bold tracking-[1.5px] uppercase text-white/60">
                Exp.
              </div>
              <div className="text-[12px] font-bold tracking-[1.5px] uppercase text-white/60 text-center">
                Actions
              </div>
            </div>
        ...
        {viewCoach && (
          <div
            className="fixed inset-0 bg-[#1A1A2E]/80 backdrop-blur-sm flex items-center justify-center z-[1000] p-5 animate-fadeIn"
          ...
          >
            <div className="bg-white rounded-[24px] w-[700px] max-w-full max-h-[90vh] overflow-y-auto shadow-[0_30px_70px_rgba(0,0,0,0.3)] animate-slideUp">
              ...
              <div className="bg-[#1A1A2E] p-[30px] rounded-t-[24px] flex items-center gap-5 relative">
                ...
                <div>
                  <div className="font-serif-custom text-[24px] text-white mb-2">
                    {viewCoach.name}
                  </div>
                  <div className="flex flex-wrap gap-[6px]">
                    {getDesig(viewCoach).map((d, i) => (
                      <span
                        key={i}
                        className="px-3 py-1 rounded-full text-[12px] font-semibold tracking-[0.3px] bg-[#F0E4C2] text-[#92640A] border border-[#E4C97C]"
                      >
                        {d}
                      </span>
                    ))}
                ...
                <button
                  className="absolute top-5 right-5 w-9 h-9 bg-white/10 rounded-[10px] text-white/70 flex items-center justify-center transition hover:bg-white/20 hover:text-white hover:scale-105"
              ...
              <div className="p-[30px]">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="bg-[#FAF7F2] border-2 border-[#E8E0D0] rounded-xl p-4">
                    ...
                    <div className="text-[16px] text-[#1A1A2E] font-medium">
                      {viewCoach.playingExperience || "—"}
                    </div>
                  </div>
                  <div className="bg-[#FAF7F2] border-2 border-[#E8E0D0] rounded-xl p-4">
                    <div className="text-[12px] font-bold tracking-[1.5px] uppercase text-[#C9A84C] mb-1.5">
                      ⏱️ Coaching Experience
                    </div>
                    <div className="text-[16px] text-[#1A1A2E] font-medium">
                      {viewCoach.coachingExperience
                        ? `${viewCoach.coachingExperience} Years`
                        : "—"}
                    </div>
                  </div>
                  <div className="bg-[#FAF7F2] border-2 border-[#E8E0D0] rounded-xl p-4">
